    ...
    case WM_COMMAND:
        ...
        case ID_SEARCH:
        {
            ...
            if (ppath != nullptr) {
                ...
                std::vector<std::wstring> files = searchFile(ppath, searchText);
                CoTaskMemFree(ppath);
                ShowWindow(GetDlgItem(hDlg, IDC_TEXT1), SW_SHOW);
                SetWindowTextW(GetDlgItem(hDlg, IDC_TEXT1), L"done");
...
public:
    CoOpenFile() : _pPath(nullptr) {};

    HRESULT init() {
        return CoInitializeEx(0, COINIT_APARTMENTTHREADED | COINIT_DISABLE_OLE1DDE);
    };

    HRESULT createInst(IFileOpenDialog *pFileOpen) {
        return CoCreateInstance(CLSID_FileOpenDialog, 0, CLSCTX_ALL, IID_IFileOpenDialog, reinterpret_cast<void**>(&pFileOpen));
    }

    WCHAR* getPPath() { return this->_pPath; };
 
    ~CoOpenFile() {
        if(_pPath != nullptr) CoTaskMemFree(_pPath);
        CoUninitialize();
    }
};
