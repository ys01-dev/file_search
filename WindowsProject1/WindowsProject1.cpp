...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		hEdit = CreateWindowEx(NULL, _T("EDIT"), NULL, WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | ES_AUTOHSCROLL | ES_AUTOVSCROLL | ES_MULTILINE,
			0, 0, rect.right - rect.left, rect.bottom - rect.top - GetSystemMetrics(SM_CYHSCROLL) - GetSystemMetrics(SM_CYMENU) - GetSystemMetrics(SM_CYCAPTION)
	...
	case WM_SIZE:
	{
		RECT rect{};
		GetWindowRect(hWnd, (LPRECT)&rect);
		SetWindowPos(hEdit, 0, 0, 0, rect.right - rect.left - GetSystemMetrics(SM_CXVSCROLL)
				, rect.bottom - rect.top - GetSystemMetrics(SM_CYHSCROLL) - GetSystemMetrics(SM_CYMENU) - GetSystemMetrics(SM_CYCAPTION), 0);
	}
	break;
	case WM_DESTROY:
		PostQuitMessage(0);
	...
	case WM_COMMAND:
		...
		case ID_SEARCH:
		{
			...
			if (ppath != nullptr) {
				...
				GetDlgItemText(hDlg, IDC_TEXTBOX, searchText, sizeof(searchText) / sizeof(WCHAR));
				std::vector<std::wstring> files;
				std::wstring message;

				try {
					files = searchFile(ppath, searchText);
					message = L"done (found " + std::to_wstring(files.size()) + L" files)";
				}
				catch (const std::exception& e) {
					WCHAR* str = new WCHAR[strlen(e.what())]{};
					size_t ret = 0;
					char* originalLC = setlocale(std::locale::ctype, "jpn");

					message = mbstowcs_s(&ret, str, strlen(e.what()), e.what(), _TRUNCATE) == NULL ? str : L"error occured";
					delete[] str;
					setlocale(std::locale::ctype, originalLC);
				}
				if (ppath != nullptr) CoTaskMemFree(ppath);

				ShowWindow(GetDlgItem(hDlg, IDC_TEXT1), SW_SHOW);
				SetWindowTextW(GetDlgItem(hDlg, IDC_TEXT1), message.c_str());
				SetWindowTextW(hEdit, !files.empty() ? joinw(files).c_str() : L"file not found");
				files.clear();
				files.shrink_to_fit();
...
std::vector<std::wstring> searchFile(std::wstring path, WCHAR* searchText) {
	...
	std::vector<std::wstring> files;
	std::wstring targetPath = L"\\\\?\\" + path + L"\\*";
	...
	if (hFile != INVALID_HANDLE_VALUE) {
		do {
			...
			if (w32fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
				if (wcscmp(pfileName, L".") == 0 || wcscmp(pfileName, L"..") == 0) continue;
...
//    std::vector<std::wstring> files;
//
//    for (const auto& file : std::filesystem::recursive_directory_iterator::recursive_directory_iterator(path, std::filesystem::directory_options::skip_permission_denied)) {
